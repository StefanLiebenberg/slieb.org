module Jekyll
  module Tags
    class HighlightFileTag < Liquid::Tag
      ...
      SYNTAX = /^([a-zA-Z0-9.\\\/]+)(\s+)([a-zA-Z0-9.+#-]+)((\s+\w+(=(\w+|"([0-9]+\s)*[0-9]+"))?)*)$/
...
eos
      ...
      def render(context)
        ...
        rendered_output = add_code_tag(output).gsub(/\n/, '<br />')
        add_filename(prefix + rendered_output + suffix)
      end

      def add_filename(code)
        "<a href=\"/#{@filename}\">#{@filename}</a><br/>#{code}"
      end
...
Liquid::Template.register_tag('file', Jekyll::Tags::HighlightFileTag)
